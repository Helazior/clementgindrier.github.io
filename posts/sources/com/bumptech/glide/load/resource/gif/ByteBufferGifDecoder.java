        ...
        try {
            ...
            if (parseHeader.getNumFrames() > 0 && parseHeader.getStatus() == 0) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    StringBuilder m253r = outline.m253r("Decoded GIF from stream in ");
                    m253r.append(LogTime.getElapsedMillis(logTime));
                    Log.v(TAG, m253r.toString());
                }
                return gifDrawableResource;
            }
            if (Log.isLoggable(TAG, 2)) {
                StringBuilder m253r2 = outline.m253r("Decoded GIF from stream in ");
                m253r2.append(LogTime.getElapsedMillis(logTime));
                Log.v(TAG, m253r2.toString());
            }
            return null;
        } finally {
            if (Log.isLoggable(TAG, 2)) {
                StringBuilder m253r3 = outline.m253r("Decoded GIF from stream in ");
                m253r3.append(LogTime.getElapsedMillis(logTime));
                Log.v(TAG, m253r3.toString());
            }
        }
    }
