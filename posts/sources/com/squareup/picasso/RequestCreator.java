...
public class RequestCreator {
    ...
    private Request createRequest(long j) {
        ...
        if (transformRequest != build) {
            transformRequest.f3936id = andIncrement;
            transformRequest.started = j;
            if (z) {
                String logId = transformRequest.logId();
                Utils.log(Utils.OWNER_MAIN, Utils.VERB_CHANGED, logId, "into " + transformRequest);
            }
        }
            ...
            if (this.data.hasImage()) {
                ...
                if (MemoryPolicy.shouldReadFromMemoryCache(this.memoryPolicy) && this.picasso.quickMemoryCacheCheck(createKey) != null) {
                    if (this.picasso.loggingEnabled) {
                        ...
                        StringBuilder m253r = outline.m253r("from ");
                        m253r.append(Picasso.LoadedFrom.MEMORY);
                        Utils.log(Utils.OWNER_MAIN, Utils.VERB_COMPLETED, plainId, m253r.toString());
                    }
                    if (callback != null) {
                        callback.onSuccess();
                        return;
    ...
    public void into(ImageView imageView, Callback callback) {
        ...
        if (imageView != null) {
            ...
            if (MemoryPolicy.shouldReadFromMemoryCache(this.memoryPolicy) && (quickMemoryCacheCheck = this.picasso.quickMemoryCacheCheck(createKey)) != null) {
                ...
                if (this.picasso.loggingEnabled) {
                    String plainId = createRequest.plainId();
                    Utils.log(Utils.OWNER_MAIN, Utils.VERB_COMPLETED, plainId, "from " + loadedFrom);
                }
                if (callback != null) {
                    callback.onSuccess();
                    return;
