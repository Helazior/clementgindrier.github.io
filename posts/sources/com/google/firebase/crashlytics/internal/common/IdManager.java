...
public class IdManager implements InstallIdProvider {
    ...
    private synchronized String createAndStoreIid(String str, SharedPreferences sharedPreferences) {
        ...
        logger.m1078v("Created new Crashlytics installation ID: " + formatId);
    ...
    private synchronized void migrateLegacyId(String str, String str2, SharedPreferences sharedPreferences, SharedPreferences sharedPreferences2) {
        Logger logger = Logger.getLogger();
        logger.m1078v("Migrating legacy Crashlytics installation ID: " + str);
    ...
    public synchronized String getCrashlyticsInstallId() {
        ...
        Logger.getLogger().m1078v("Determining Crashlytics installation ID...");
        ...
        try {
            ...
            Logger.getLogger().m1075w("Failed to retrieve Firebase Installations ID.", e);
            str = string != null ? string : null;
        }
        if (string == null) {
            Logger.getLogger().m1078v("No cached Firebase Installations ID found.");
            ...
            if (string2 == null) {
                Logger.getLogger().m1078v("No legacy Crashlytics installation ID found, creating new ID.");
                this.crashlyticsInstallId = createAndStoreIid(str, sharedPrefs);
            } else {
                Logger.getLogger().m1078v("A legacy Crashlytics installation ID was found. Upgrading.");
            ...
            Logger.getLogger().m1078v("Firebase Installations ID is unchanged from previous startup.");
            if (this.crashlyticsInstallId == null) {
                Logger.getLogger().m1078v("Crashlytics installation ID was null, creating new ID.");
                this.crashlyticsInstallId = createAndStoreIid(str, sharedPrefs);
            }
        } else {
            this.crashlyticsInstallId = createAndStoreIid(str, sharedPrefs);
        ...
        Logger logger = Logger.getLogger();
        logger.m1078v("Crashlytics installation ID is " + this.crashlyticsInstallId);
