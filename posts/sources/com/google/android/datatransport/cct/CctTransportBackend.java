...
public final class CctTransportBackend implements TransportBackend {
    ...
    public HttpResponse doSend(HttpRequest httpRequest) {
        Logging.m1115d(LOG_TAG, "Making request to: %s", httpRequest.url);
        ...
        httpURLConnection.setRequestProperty(DefaultSettingsSpiCall.HEADER_USER_AGENT, String.format("datatransport/%s android/", BuildConfig.VERSION_NAME));
        ...
        try {
            ...
            try {
                ...
                Logging.m1111i(LOG_TAG, "Status Code: " + responseCode);
                Logging.m1111i(LOG_TAG, "Content-Type: " + httpURLConnection.getHeaderField(CONTENT_TYPE_HEADER_KEY));
                Logging.m1111i(LOG_TAG, "Content-Encoding: " + httpURLConnection.getHeaderField(CONTENT_ENCODING_HEADER_KEY));
            ...
            e = e;
            Logging.m1112e(LOG_TAG, "Couldn't encode request, returning with 400", e);
            return new HttpResponse(400, null, 0L);
        } catch (ConnectException e2) {
            e = e2;
            Logging.m1112e(LOG_TAG, "Couldn't open connection, returning with 500", e);
    ...
    private static int getPackageVersionCode(Context context) {
        try {
            ...
            Logging.m1112e(LOG_TAG, "Unable to find version code for package", e);
            return -1;
        }
    }
    ...
    private BatchedLogRequest getRequestBody(BackendRequest backendRequest) {
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            ...
            for (EventInternal eventInternal3 : (List) entry.getValue()) {
                ...
                if (encoding.equals(Encoding.m1118of("proto"))) {
                    protoBuilder = LogEvent.protoBuilder(encodedPayload.getBytes());
                } else if (encoding.equals(Encoding.m1118of("json"))) {
                    protoBuilder = LogEvent.jsonBuilder(new String(encodedPayload.getBytes(), Charset.forName(Key.STRING_CHARSET_NAME)));
                } else {
                    Logging.m1110w(LOG_TAG, "Received event of unsupported encoding %s. Skipping...", encoding);
                }
    ...
    public static /* synthetic */ HttpRequest lambda$send$0(HttpRequest httpRequest, HttpResponse httpResponse) {
        ...
        if (url != null) {
            Logging.m1115d(LOG_TAG, "Following redirect to: %s", url);
    ...
    private static URL parseUrlOrThrow(String str) {
        try {
            ...
            throw new IllegalArgumentException(outline.m266e("Invalid url: ", str), e);
        }
    }
    ...
    public BackendResponse send(BackendRequest backendRequest) {
        ...
        try {
            ...
            if (i == 200) {
                return BackendResponse.m1117ok(httpResponse.nextRequestMillis);
            }
            if (i < 500 && i != 404) {
                return BackendResponse.fatalError();
            }
            ...
            Logging.m1112e(LOG_TAG, "Could not make request to the backend", e);
            return BackendResponse.transientError();
        }
    }
