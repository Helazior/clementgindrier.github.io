...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) {
        ...
        if (request.header(DefaultSettingsSpiCall.HEADER_USER_AGENT) == null) {
            newBuilder.header(DefaultSettingsSpiCall.HEADER_USER_AGENT, Version.userAgent());
        }
        Response proceed = chain.proceed(newBuilder.build());
        HttpHeaders.receiveHeaders(this.cookieJar, request.url(), proceed.headers());
        Response.Builder request2 = proceed.newBuilder().request(request);
        if (z && "gzip".equalsIgnoreCase(proceed.header("Content-Encoding")) && HttpHeaders.hasBody(proceed)) {
            C2220re c2220re = new C2220re(proceed.body().source());
            request2.headers(proceed.headers().newBuilder().removeAll("Content-Encoding").removeAll("Content-Length").build());
            String header = proceed.header("Content-Type");
            Logger logger = C2263te.f6595a;
            request2.body(new RealResponseBody(header, -1L, new C2358xe(c2220re)));
        }
