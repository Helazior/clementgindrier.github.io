...
public class NetscapeCertRequest extends ASN1Object {
    ...
    public boolean verify(String str) {
        if (str.equals(this.challenge)) {
            Signature signature = Signature.getInstance(this.sigAlg.getObjectId().getId(), "SC");
        ...
        try {
            if (aSN1Sequence.size() == 3) {
                ...
                if (aSN1Sequence2.size() == 2) {
                    ...
                    this.pubkey = KeyFactory.getInstance(algorithmId.getObjectId().getId(), "SC").generatePublic(x509EncodedKeySpec);
                    return;
                }
    ...
    public void sign(PrivateKey privateKey, SecureRandom secureRandom) {
        Signature signature = Signature.getInstance(this.sigAlg.getAlgorithm().getId(), "SC");
